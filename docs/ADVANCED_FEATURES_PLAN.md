...
**Status:** Phase 1 is built and shipped on this branch. Phases 2–5 remain a proposal.
...
| Item                         | What shipped                                                                                                                            |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| Two-input engine             | `EngineOperation` carries multi-input operations; `estimateSize` counts every side, so a pair of large documents offloads to the worker |
| **A1** JSON/YAML diff        | `/json-diff`, `/yaml-diff` — structural comparison, three array-pairing strategies, RFC 6902 and RFC 7386 patch export                  |
| **B4** Fix-it suggestions    | String-aware repair for 8 classes of broken JSON, previewed and never auto-applied                                                      |
| **C2** Presets               | Five built-in presets plus user-saved ones                                                                                              |
| **B6** Deep-linkable options | Settings mirrored into the query string; options only, never document contents                                                          |
| **B3** Auto-detect           | Recognises JSON, NDJSON, YAML, XML, CSV/TSV, TOML and points at the right tool                                                          |

Two refactors came with it: `ToolPageShell` (the SEO page shell, previously duplicated per bespoke
tool page) and `src/lib/value.ts` (shared value labelling for the tree view and diff).

**Not yet built from Phase 1:** A2, the JSONPath / jq query playground — the one remaining item, and
the natural next piece of work.

Test coverage across the phase: 190 unit tests and 23 Playwright specs, all green.
...
**A1. JSON / YAML Diff & Merge** — _Impact: acquisition + retention · Effort: L_
...
`/json-diff`, `/yaml-diff`.
_Why first:_ "json diff" and "compare json" are among the largest untapped search volumes adjacent
...
**A2. JSONPath & jq-style query playground** — _Impact: acquisition + retention · Effort: M_
...
**A3. Schema inference — generate a JSON Schema from sample data** — _Impact: acquisition + revenue · Effort: M_
...
**A4. Type codegen — JSON/Schema → TypeScript, Go, Python, Rust, Java, C#** — _Impact: acquisition · Effort: M_
...
**A5. The converter matrix — CSV, XML, TOML, .env, .properties, Base64, query strings** — _Impact: acquisition · Effort: M per pair, S after scaffolding_
...
**A6. JSON Graph view (node-and-edge visualization)** — _Impact: acquisition + shareability · Effort: L_
The JSON Crack effect: a canvas graph of the document with pan/zoom, collapsible subgraphs, and PNG/SVG
export. This is the most _screenshottable_ feature in the category — it's what gets posted on Reddit
and Hacker News. Route: `/json-graph`.

**A7. Adjacent micro-tools** — _Impact: acquisition · Effort: S each_
...
**B1. Multi-document workspace with tabs** — _Impact: activation + retention · Effort: L_
...
**B2. Command palette (⌘K)** — _Impact: activation + retention · Effort: M_
...
**B3. Auto-detect input format** — _Impact: activation · Effort: S_
...
**B4. Fix-it suggestions with one-click apply** — _Impact: activation · Effort: M_
We already report precise errors and deliberately never repair silently. Keep that, and add
_explicit, previewed_ fixes: trailing comma, single→double quotes, unquoted keys, tab/space mixing,
...
**B5. Real virtualization for large documents** — _Impact: activation + differentiation · Effort: L_
...
**B6. Deep-linkable tool state (no document)** — _Impact: acquisition · Effort: S_
Encode _options_ — not content — in the URL: `?indent=4&sort=true`. Shareable, bookmarkable, and it
gives teams a way to standardize settings without an account.

### Tier C — Retention: reasons to come back and to stay

**C1. Local history & snippet library** — _Impact: retention · Effort: M_
...
**C2. Presets / profiles** — _Impact: retention · Effort: S_
...
**C3. Custom anonymization rules** — _Impact: retention + revenue · Effort: M_
...
**C4. End-to-end encrypted sharing** — _Impact: retention + virality + revenue · Effort: L_
...
This is the _only_ way to add sharing without breaking our promise — and it's a promise we can state
precisely, which makes it marketable. Requires a storage backend and a CSP amendment; both scoped to
ciphertext only.

**C5. Mock / fake data generator from a schema** — _Impact: retention · Effort: M_
...
**C6. Offline-first, genuinely** — _Impact: retention · Effort: S–M_
...
**D1. `npx` CLI + npm library** — _Impact: retention + revenue + credibility · Effort: M_
...
**D2. GitHub Action** — _Impact: retention (deep) · Effort: S once D1 exists_
...
**D3. VS Code extension** — _Impact: retention · Effort: M_
...
**D4. Browser extension** — _Impact: acquisition + retention · Effort: M_
...
**D5. Docs, embeds, and an OSS posture** — _Impact: acquisition · Effort: M_
An embeddable read-only viewer widget (`<iframe>` or web component) that others put in _their_ docs,
...
**E1. Accounts + Stripe Checkout** — _Effort: M_
...
**E3. Team schema registry** — _Effort: L_
...
**E4. Pro tier, defined concretely** — _Effort: S once E1 ships_
...
priority support. Every one of those is a _more of what you already like_ upgrade, not a
_we took something away_ upgrade.

---

## 5. Phased delivery plan

**Phase 1 — Prove the thesis (~6 weeks).** ✅ A1 diff · ⬜ A2 query · ✅ B3 auto-detect ·
✅ B4 fix-it · ✅ C2 presets · ✅ B6 deep-linkable options.
_Exit criteria:_ two new high-volume search terms ranked; measurable lift in returning visitors.
_Status:_ five of six shipped; A2 outstanding. The exit criteria are measurements to take after
`/json-diff` and `/yaml-diff` have been live long enough to rank — they are not yet met, and
Phase 2 should not start until they are.
...
_Exit criteria:_ ≥25% of sessions are returning users; median session touches ≥2 tools.

**Phase 3 — Escape the tab (~8 weeks).** D1 CLI · D2 GitHub Action · D4 browser extension ·
A5 converter matrix · A7 micro-tools.
_Exit criteria:_ CLI installs and Action usage tracked as first-class top-of-funnel.

**Phase 4 — Charge for it (~8 weeks).** E1 accounts + Stripe · E4 Pro defined and live ·
C3 anonymization rule sets · C4 E2E sharing.
_Exit criteria:_ first paying customers; free tier demonstrably not degraded.
...
- ~~**Two-input operations.**~~ **Done.** `EngineOperation` now carries multi-input operations and
  `runner.estimateSize` accounts for every input, so both sides of a comparison count toward the
  worker-offload threshold. Doing this before A1 rather than after was the right call.
...
1. Document contents never leave the device except through C4, where they leave _encrypted with a
   key the server never sees_, only on an explicit user action.
...
_Prepared as a planning document. No product code changes accompany it._
